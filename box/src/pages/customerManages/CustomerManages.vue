  ...
  <div class="user">
    <div class="formData">
      <el-card class="searchModule">
        <el-form label-width="100px" :model="user" :inline="true" :rules="rulsa" :ref="user">
          <el-form-item label prop="searchValue">
            <el-input v-model="user.searchValue" placeholder="客户名/姓名/手机号/邮箱" clearable></el-input>
          </el-form-item>
    ...
    <CustomerEdit ref="editData" v-on:search="search"></CustomerEdit>
    <el-dialog
      title="修改密码"
        ...
        if (0 != this.multipleSelection.length) {
          this.$confirm("是否启用？", "提示", {
            ...
            .then(() => {
              ...
              param.userIds = param.userIds.join(",");
              this.$axios
                .put("prod-api/backend/customer/enable/" + param.userIds, param)
                .then(res => {
                  if (200 == res.data.code) {
                    this.$message({
                      showClose: true,
        ...
        if (0 != this.multipleSelection.length) {
          this.$confirm("是否禁用？", "提示", {
            ...
            .then(() => {
              ...
              this.$axios
                .put(
                  "prod-api/backend/customer/disable/" + param.userIds,
                  param
                )
                .then(res => {
                  if (200 == res.data.code) {
        ...
        if (0 != this.multipleSelection.length) {
          this.$confirm("此操作将永久删除改数据", "是否继续？", "提示", {
            ...
            .then(() => {
              ...
              this.$axios
                .delete(
                  "prod-api/backend/customer/delete/" + param.userIds,
                  param
                )
                .then(res => {
                  if (200 == res.data.code) {
